...
- Colima (0.6.14) - Docker VM für macOS
- Alle Container-Images

**Keine Homebrew, keine Docker Desktop nötig!**
...
./setup.sh --ai     # Mit AI
./ai-setup.sh       # AI nachträglich aktivieren
...
nano backup.sh  # /Volumes/ExterneFestplatte ändern
```
...
### Passwort ändern
Web-UI: Settings → Users → admin → Change Password

### API-Token erstellen
Web-UI: Settings → API Tokens → Create
...
2. Automatische Vorschläge nutzen
3. Nur noch korrigieren statt neu taggen
4. Regelmäßig Backup: `./backup.sh`
...
**OCR erkennt nichts:**
- PDF-Qualität prüfen (min. 200 DPI)
- Sprache prüfen: Settings → OCR → Languages
- Tika-Logs: `docker compose logs tika`

**System langsam:**
- Workers erhöhen (siehe Performance)
- AI-Profil deaktivieren: `docker compose stop ollama paperless-ai`
- Nur nötige Container laufen lassen

**Zu viel Festplatte:**
- Alte Exports löschen: `rm -rf export/backup-*`
